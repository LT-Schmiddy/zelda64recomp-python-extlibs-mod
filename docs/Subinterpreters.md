...
REPY has API functionality available so that subinterpreters can be easily used within N64Recompiled mod code, leveraging subinterpreter control functions in the Python C API and instead of using Python 3.14's `concurrent.interpreters` module. This means that subinterpreters won't suffer a performance penalty when compared to the main interpreter.
