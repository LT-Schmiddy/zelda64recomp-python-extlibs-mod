...
// Variadic Locals Handling. This function is always followed by one of the
// others in this category, therefore storing the va_locals using with thread_local
// works without issue.
thread_local py::dict va_locals;

RECOMP_DLL_FUNC(PythonNative_ConstructVL) {
    controller->set_rdram(rdram);
    py::gil_scoped_acquire gil;

    if (RECOMP_ARG(REPY_Handle, 0) != 0) {
        va_locals = *((py::dict*)RECOMP_ARG_PYOBJECT(0));
    } else {
        va_locals = py::dict();
    }

    int32_t count = RECOMP_ARG(int32_t, 1);
    REPY_Handle* va_args_ptr = RECOMP_ARG(REPY_Handle*, 2);
    
    for (int i = 0; i < count; i++) {
        va_locals[py::str(std::format("_{}", i))] = controller->get_py_object(va_args_ptr[i]);
    }
}

RECOMP_DLL_FUNC(PythonNative_EvalVL) {
    controller->set_rdram(rdram);
    py::gil_scoped_acquire gil;
    py::object* bytecode = RECOMP_ARG_PYOBJECT(0);
    py::dict empty_globals = py::dict();
    ...
    py::object result;
    try {
        result = controller->py_eval(bytecode, globals, va_locals);
    } catch (py::error_already_set &e) {
        controller->handle_exception(&e);
        controller->release_suh_handles();
        RECOMP_RETURN(REPY_Handle, 0);
...
RECOMP_DLL_FUNC(PythonNative_EvalVLCStr) {
    controller->set_rdram(rdram);
    py::gil_scoped_acquire gil;
    std::string code_string = RECOMP_ARG_STR(0);
    py::dict empty_globals = py::dict();
    ...
    py::object result;
    try {
        result = controller->py_eval(code_string, globals, va_locals);
    } catch (py::error_already_set &e) {
        controller->handle_exception(&e);
        controller->release_suh_handles();
        RECOMP_RETURN(REPY_Handle, 0);
...
RECOMP_DLL_FUNC(PythonNative_EvalVLCStrN) {
    controller->set_rdram(rdram);
    py::gil_scoped_acquire gil;
    int32_t code_len = RECOMP_ARG(int32_t, 1);
    std::string code_string = RECOMP_ARG_STR_N(0, code_len);
